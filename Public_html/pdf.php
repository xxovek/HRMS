...
$CInfo_res = mysqli_query($con, $fetchCompanyInfo) or die("Company info not Fetched");
if (mysqli_num_rows($CInfo_res) > 0) {
  ...
  if ($CInfo_row['logoImage'] == 'defaultlogo.png') {
    $CmpInfoTr = '<tr><th colspan="6" style="text-align:center">' . $CInfo_row['companyname'] . '<br>' . $Caddress . '</th></tr>';
  } else {
    $CmpInfoTr = '<tr>
...
$months_arr= array('dum', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec');
// YEAR(`fromDate`)='$yr' AND MONTH(`fromDate`)='$month'

for ($i = 1; $i <= 12; $i++) {
  if ($month == $i) {
     $SalaryMonth1 = $months_arr[$i];
    break;
  }
}

$SalaryMonth = $SalaryMonth1 . "-" . $yr;

// $total=$totDed=$totEar=0;
$sql_query = " SELECT * FROM Employees e,EmployeeSalaryPayslip ep,SalaryHeads s
   WHERE e.EmpId = ep.EmpId AND ep.SalaryHeadId = s.SalaryHeadId AND YEAR(GeneratedDate)='$yr'
    AND MONTH(GeneratedDate)='$month'
    AND e.EmpId= $Emp_id ";
// echo $sql_query;
$result  = mysqli_query($con, $sql_query) or die("Employee Info not Fetched");
$tr = '';
...
WHERE E.EmpId  = $Emp_id";
  ...
  // $tr2 = '';
  $res = mysqli_query($con, $fetchDep_query) or die("Department not Fetched");
  ...
  $PFInfoRes = mysqli_query($con, $fetchPFInfo) or die("Error in Fetching Emp PF Details");
...
AND  e.EmpId= $Emp_id and s.CredDebit = 'D'";
  ...
  // FROM EmployeeSalaryStructure,SalaryHeads WHERE '$yr' BETWEEN YEAR(fromDate) AND YEAR(uptoDate) AND EmpId = $Emp_id AND SalaryHeads.SalaryHeadId = EmployeeSalaryStructure.SalaryHeadId AND SalaryHeads.CredDebit = 'D'";
